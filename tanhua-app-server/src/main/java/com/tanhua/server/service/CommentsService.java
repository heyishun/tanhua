...
public class CommentsService {
     ...
     * @return
     */
    public Integer likeMComment(String commendId) {
        Integer count = commentApi.hasMComment(commendId);
        //拼接redis的key
        String key = Constants.MOVEMENTS_INTERACT_KEY + commendId;
        String hashKey = Constants.MOVEMENT_LOVE_HASHKEY + UserHolder.getUserId();
        redisTemplate.opsForHash().put(key,hashKey,"1");
        return count;
    }
     ...
     * @return
     */
    public Integer dislikeMComment(String commendId) {
        Integer count = commentApi.disHasMComment(commendId);
        //拼接redis的key
        String key = Constants.MOVEMENTS_INTERACT_KEY + commendId;
        String hashKey = Constants.MOVEMENT_LOVE_HASHKEY + UserHolder.getUserId();
        redisTemplate.opsForHash().delete(key,hashKey);
