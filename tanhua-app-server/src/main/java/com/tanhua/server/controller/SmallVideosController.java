...
public class SmallVideosController {
    ...
    public ResponseEntity focus(@PathVariable("uid") Long uid){
        videosService.focus(uid);
        return ResponseEntity.ok(null);
    }
    /**
     * 取消关注用户
    ...
    public ResponseEntity unFocus(@PathVariable("uid") Long uid){
        videosService.unFocus(uid);
        return ResponseEntity.ok(null);
    }
}